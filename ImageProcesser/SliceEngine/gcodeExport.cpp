...
void GCodeExport::setExtruderOffset(int id, Pointc p)
{
    extruderOffset[id] = p;
}
...
Pointc GCodeExport::getPositionXY()
{
    return Pointc(currentPosition.x, currentPosition.y);
}
...
void GCodeExport::writeMove(Pointc p, int speed, int lineWidth)
{
    if (flavor == GCODE_FLAVOR_BFB)
    {
        ...
        //如果不是在移动，则将挤出的长度写入到Gcode中
        //在这里，可以根据颜色来判断各个挤出头需要挤出的数量
        if (lineWidth != 0)
...
void GCodePlanner::addTravel(Pointc p)
{
...
void GCodePlanner::addExtrusionMove(Pointc p, GCodePathConfig* config)
{
...
{
    Pointc p0 = polygon[startIdx];
    addTravel(p0);
    for(unsigned int i=1; i<polygon.size(); i++)
    {
        Pointc p1 = polygon[(startIdx + i) % polygon.size()];
...
void GCodePlanner::writeGCode(bool liftHeadIfNeeded, int layerThickness)
{
    ...
    for(unsigned int n=0; n<paths.size(); n++)
    {
        ...
        if (path->config->lineWidth != 0)// Only apply the extrudeSpeedFactor to extrusion moves
            speed = speed * extrudeSpeedFactor / 100;   //喷头的速度
        else
            speed = speed * travelSpeedFactor / 100;    //不出料时，喷头移动的速度
        
        //移动的点的距离，也就是打印时，出丝的那些段
        if (path->points.size() == 1 && path->config != &travelConfig && shorterThen(gcode.getPositionXY() - path->points[0], path->config->lineWidth * 2))
        {
            ...
            if (i > n + 2)
            {
                ...
                for(unsigned int x=n; x<i-1; x+=2)
                {
                    ...
                    Pointc newPoint = (paths[x].points[0] + paths[x+1].points[0]) / 2;
    ...
    if (liftHeadIfNeeded && extraTime > 0.0)
    {
        ...
        gcode.writeMove(gcode.getPositionXY() - Pointc(-MM2INT(20.0), 0), travelConfig.speed, 0);
